...
{
    NeffTB(G.ListBangunan)++;
    ElmtTB((NeffTB(G.ListBangunan))) = B;
}
...
/* Pencarian indeks bangunan */
{
    boolean found = false;
    int i = 1;
    while(!found && i<= NeffTB(G.ListBangunan)){
        if(Row(ElmtTB(i).posisi) == Baris && Col(ElmtTB(i).posisi) == Kolom){
            found = true;
        }
    }
    if(i <= NeffTB(G.ListBangunan)){
        return i;
    } else {
...
void TulisIsiTabBangunan(){
    ...
    for(int i = 1; i <= NbBangunan(G.ListBangunan); ++i){
        ...
        if (i != NbBangunan(G.ListBangunan)) printf(",");
    }
    printf("]");
}
